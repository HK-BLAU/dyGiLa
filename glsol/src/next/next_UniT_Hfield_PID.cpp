...
void glsol::next_UniT_Hfield_PID() {
  ...
  bool is_diffusive = (t < config.tdif);
  bool is_dissipative = (!is_diffusive && t < config.tdis && config.gamma.squarenorm() > 0);
  ...
  onsites(ALL) {
    ...
    if (is_diffusive) {
      ...
      pi[X] = deltaPi[X]/(config.difFac);
    }
    else if (is_dissipative)
      {
        pi[X] += (deltaPi[X] - 2.0 * config.gamma * pi[X])*(config.dt);
      }
    else
      {
        pi[X] = deltaPi[X]*config.dt;
      }
  }
